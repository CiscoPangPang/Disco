...
def OnWriteReg(command) :
    ...
    if reg_val.startswith('0x') :
        reg_val = hex(int(reg_val, 16))[2:]
        reg_val = reg_val.strip('L')
    else :
...
def OnGdbContinue() :
    ...
    while True:
        ...
        elif concmd == 'debug' or concmd == 'dbg' :
            if IsAlive() == 'continue' :
                ser.write('\n')    
                ...
                print 'Router\'s status is already "Debug"'
                return True
...
def OnSetCiscoBase():
    ...
    recv = ser.read_until("{}#".format(host))
    buf = recv.split('\r\n')
    for lexem in buf :
        if '0x' not in lexem :  
            continue
    ...
    buf = buf[8]
    ser.read()
    CISCO_BASE_ADDR = int(buf.split(" ")[1], 16) - IDA_OFFSET
...
def OnLoadInfo(): # save IOS information just like break point, Cisco IOS Router base address and memory map 
    ...
    if CISCO_BASE_ADDR == None :
        CISCO_BASE_ADDR = int(bp_info[0], 16)
    elif CISCO_BASE_ADDR != int(bp_info[0], 16) :
        return
    ...
    for i in range(2, 13) :
        ...
        mapping_cnt = i
        try :
            memory['start'] = int(tmp[0].replace('L', ''), 16)
            memory['end'] = int(tmp[1].replace('L', ''), 16)
            memory['size'] = int(tmp[2].replace('L', ''))
            memory['class'] = tmp[3]
            memory['media'] = tmp[4]
            memory['name'] = tmp[5]
            memory_map.append(memory)
            ...
            break
    for i in range(mapping_cnt + 1, len(bp_info) - 1, 3) :
...
def OnSaveInfo() :
    ...
    for i in memory_map :
        buf += '{} '.format(hex(i['start']))
        buf += '{} '.format(hex(i['end']))
        buf += '{} '.format(str(i['size']))
        buf += '{} '.format(i['class'])
        buf += '{} '.format(i['media'])
        buf += '{}\r\n'.format(i['name'])
...
def OnVmmap(command) :
    ...
    if len(lexem) == 2:
        try :
            ...
            print e,
            print ' : %s is not address.'%lexem[1]
            return

    print 'START      END        SIZE       CLASS   MEDIA  NAME'
    if not addr :
        for i in memory_map :
            print '0x%08x 0x%08x 0x%08x %- 7s %- 6s %s'%(i['start'], i['end'], i['size'], i['class'], i['media'], i['name'])

    else :
        for i in memory_map :
            if i['start'] <= addr and addr <= i['end']:
                print '0x%08x 0x%08x 0x%08x %- 7s %- 6s %s'%(i['start'], i['end'], i['size'], i['class'], i['media'], i['name'])
    ...
    while True :
        try :
            ...
            if command == 'exit' :
                ...
                OnListBreak()
            elif command.startswith('setreg') or command.startswith('set') :
                OnWriteReg(command)
