...
def main():
    ...
    with open(string_offset_fname, "r") as f:
        ...
        for func_pair in buf:
            ...
            str_func_list.append({"name": func_name, "offset":int(func_offset, 16)})
        ...
        try:
            ...
            for str_funcs in str_func_list:
                if str_funcs["offset"] == func_offset:
                    print "[!] offset 0x%08x is %s! pass it for mental health" % (func_offset, str_funcs["name"])
                    isBanned = True
                    break
        ...
        try:
            ...
            if IsAlive() == "debug":
                ...
                if idle_addr in breakpoints:
                    ...
                    odd_bp_addr = "%08x" % (pc_offset + base_addr + 4) # fxxking MIPS pipe lining 1
        ...
        try:
            ...
            if IsAlive() == "debug":
                ...
                if pc in breakpoints:
                    ...
                    pc = "%08x" % (pc_offset + base_addr + 4) # fxxking MIPS pipe lining 2
                    if pc in breakpoints:
                        OnWriteMem(pc, breakpoints[pc][1])
                        OnStepInto()
                        # time.sleep(0.0001)
