...
func_offset_file = "ftrace_func_list.txt"
...
for i in range(1, 33):
    ...
    if i == 32:
        restore_lw += mips_asm("lw $t7, ($t2)") # $t7 will move its value to $ra at go_back_home
    else:
...
for i in range(1, 33):
    ...
    """ % (i, (520 - (0x10 * i)))).replace("00000000", "")
    if i == 32:
        restore_sw += mips_asm("sw $t7, ($t2)") # $t7 store return address of original function
    else:
...
func_prologue_patch = ""
func_prologue_patch += mips_asm("move $t7, $ra")
...
def get_text_addr():
    ...
    buf = [x for x in ser.read_until("main:text").split("\r\n") if "IText" in x][1]

    # 0x3000CE68  0x368F762B   110012356  IText  R/O    main:text
    main_text_start, main_text_end = [int(x, 16) for x in buf.split(" ") if x.startswith("0x")][:2]

    # buf = [x for x in ser.read_until("main:bss").split("\r\n") if "IBss" in x][0]
    # reg_store_addr = [int(x, 16) for x in buf.split(" ") if x.startswith("0x")][0]
    reg_store_addr = 0x3fffff00
    
    base_addr = main_text_start - IDA_OFFSET
    printf_addr = (printf_addr - IDA_BASE_ADDR) + base_addr
    noppad_addr = (noppad_addr - IDA_BASE_ADDR) + base_addr
    noppad_size = main_text_end - noppad_addr

    # I love aligned address as much as mmap()
    if noppad_addr and 0xF:
...
def parse_args():
    ...
    parser.add_argument("-f", "--funclist", type=str, required=False, help="file that store function offset list( default = ftrace_func_list.txt )")
    parser.add_argument("--idabase", type=str, required=False, help="IDA Imagebase address of Cisco IOS( default = 0x30000000 )")
...
def main():
    ...
    # Now, you have to set dynamic address
    dyn_addr_list = []

    # call_printf = fmt_str_addr, jal printf
    dyn_addr_list.append(0x10000-(base_addr - IDA_BASE_ADDR))
    ...
    print "[+] Please attach debugger and continue!"
    #GdbCommand("c")
    ser.close()
